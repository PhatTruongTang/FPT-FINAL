// ===== Neon Shooter - FULL MAIN (Weapons + Save/Load Upgrades Only) =====
// - Lưu/Load: chỉ lưu "Points" + "Upgrades" vào localStorage.
// - Vũ khí: Bullet cơ bản + Missile homing + Laser beam.
// - Shop: nâng cấp cho Bullet / Missile / Laser, trừ dần Points.
// - Trạng thái: 'menu' | 'playing' | 'gameover'. P2 tắt ở Single Mode.
// - Tích hợp BGM/SFX nếu có window.audio, an toàn khi thiếu.
...
function saveSave(obj) {
  try { localStorage.setItem(SAVE_KEY, JSON.stringify({ totalPoints: obj.totalPoints|0, upgrades: obj.upgrades })); } catch {}
}
let save = loadSave();
...
function toMenuBGM(){ try{ window.audio?.toMenu(); }catch{} }
function toGameBGM(){ try{ window.audio?.toGame(); }catch{} }
...
// helper explosion visuals
function spawnExplosion(x,y,sz,isBoss){
  for(let k=0;k<12+(isBoss?16:0);k++) sparkle(x+(Math.random()-0.5)*sz, y+(Math.random()-0.5)*sz, 8+Math.random()*12);
  for(let k=0;k<6+(isBoss?10:0);k++) puff(x,y, sz*0.5 + Math.random()*sz*0.6, 'rgba(255,240,200,0.25)');
  sfxExpl(isBoss?'big': (sz>22?'big': sz>12?'medium':'small'));
}
...
class Particle {
  ...
  draw(ctx){
    ...
    ctx.globalAlpha=a; ctx.fillStyle=this.clr;
    ctx.beginPath(); ctx.arc(this.x,this.y,this.size*a,0,Math.PI*2); ctx.fill(); ctx.globalAlpha=1;
  }
}
let particles=[]; 
function puff(x,y,color,count=10,spd=2,life=0.7,size=3){
  for(let i=0;i<count;i++){
    const a=Math.random()*Math.PI*2, s=rand(spd*0.3,spd);
    particles.push(new Particle(x,y,Math.cos(a)*120*s,Math.sin(a)*120*s, rand(size*0.5,size), life, color));
  }
}
...
function makePlayer(id,x,y,skin,hue){
  return {
    ...
    bulletDmg:8, // base
    // missiles
    missileUnlocked:false, missileRate:0.8, missileReload:0,
    missileSpeed:230, missileHoming:1.0, missileBlast:40, missileDamage:6,
    // laser
    laserUnlocked:false, laserRate:0.6, laserCooldown:0,
    laserDuration:0.6, laserTick:0.1, laserDamage:12, laserSize:8, laserPierce:false,
    score:0
  };
}
const players = [
...
// (Optional) Boss — giữ nhẹ cho ngắn gọn
class Boss{
  constructor(){
    ...
    this.t=0; this.phase=1; this.shoot=1.2;
  }
  update(dt){
...
// ---------------------- Weapons firing helpers ----------------------
function shootBullets(p, ang){
  // 3 viên xiên nhẹ như bản gốc “chân thật”
  for(let s=-1;s<=1;s++){
    const a = ang + s*0.06;
    bullets.push({ x:p.x + Math.cos(a)*20, y:p.y + Math.sin(a)*20, vx:Math.cos(a)*760, vy:Math.sin(a)*760, size:5, hue:p.colorHue, hostile:false, owner:'p'+p.id });
  }
  puff(p.x + Math.cos(ang)*16, p.y + Math.sin(ang)*16, 'rgba(180,255,255,0.9)', 10, 2.0, 0.35, 3);
  sfxShot();
}
...
function update(dt){
  ...
  if(gameState==='playing' && p1.active && p1.alive){
    ...
    if((mouse.left || keys[' ']) && p1.reload<=0){
      p1.reload = p1.fireRate;
      shootBullets(p1, aimAng);
    }
    // missiles auto
    fireMissileIfReady(p1, dt);
    // laser (nhấn chuột phải)
    p1.laserCooldown -= dt;
    if(mouse.right && p1.laserUnlocked && p1.laserCooldown<=0){
      lasers.push({ owner:'p1', x:p1.x,y:p1.y,tx:mouse.x,ty:mouse.y,age:0,duration:p1.laserDuration });
  ...
  if(gameMode==='coop' && p2.active && p2.alive && gameState==='playing'){
    ...
    if(keys['m'] && p2.reload<=0){
      ...
      shootBullets(p2, ang2);
    }
    fireMissileIfReady(p2, dt);
  }

  // movement damping / bounds
  ...
  for(let i=missiles.length;i--;){
    ...
    if(target){
      ...
      m.vx += (tx-m.vx)*0.02*hom;
      m.vy += (ty-m.vy)*0.02*hom;
    }
    ...
    for(let j=enemies.length;j--;){
      const e=enemies[j];
      const dx=e.x-m.x, dy=e.y-m.y, rr=(e.rad+6); 
    ...
    if(hit>=0 || m.age>6){
      ...
      for(let j=enemies.length;j--;){
        ...
        if(dx*dx+dy*dy <= R*R){
          e.hp -= DMG;
          if(e.hp<=0){ enemies.splice(j,1); (P.score=(P.score||0)+1); save.totalPoints+=2; }
        }
      }
      puff(m.x,m.y,'rgba(255,200,120,0.95)',18,3.2,0.9,1.1);
      sfxExpl('medium');
      missiles.splice(i,1);
    }
  }
...
for(let i=lasers.length;i--;){
  const L=lasers[i];
  L.age += dt;
  if(L.age > L.duration){ lasers.splice(i,1); continue; }
  // identify owner and make laser stick to ship
  let ownerIdx = 0;
  if(L.owner==='p2') ownerIdx = 1;
  const P = players[ownerIdx] || players[0];
  // follow player's muzzle each frame
  const mx = Math.cos(P.dir), my = Math.sin(P.dir);
  const muzzle = 22;
  L.x = P.x + mx*muzzle;
  L.y = P.y + my*muzzle;
  // compute beam end target
  const maxLen = 1600;
  L.tx = L.x + mx*maxLen;
  L.ty = L.y + my*maxLen;

  // Collision: either single target (no pierce) or multi (pierce unlocked)
  const r = P.laserSize + 6;
  const r2 = r*r;
  const px = L.x, py = L.y;
  const vx = mx, vy = my;

  let firstT = Infinity, firstIdx = -1, firstPoint = null;
  let farthestT = 0;
  L._cool = L._cool || {};
  ...
  for(let j=0;j<enemies.length;j++){
    ...
    const t = Math.max(0, Math.min(maxLen, ex*vx + ey*vy)); // projection length
    const hx = px + vx*t, hy = py + vy*t;
    const dx = E.x - hx, dy = E.y - hy;
    if(dx*dx + dy*dy <= (E.size+6)*(E.size+6)){
      // record
      if(!P.laserPierce){
        if(t < firstT){ firstT = t; firstIdx = j; firstPoint = {x:hx, y:hy}; }
      }else{
        // throttle per enemy
        const now = tNow;
        if(L._cool[j] && now < L._cool[j]) continue;
        L._cool[j] = now + 0.05;
        E.hp -= P.laserDamage;
        hitFlash(E.x,E.y, E.size*0.7);
        if(E.hp<=0){
          spawnExplosion(E.x,E.y, E.size);
          addScore(E.value||10);
          enemies.splice(j,1); j--;
        }
        if(t>farthestT) farthestT = t;
      }
    }
  }
  // hit boss (single large target)
  for(let b=0;b<bosses.length;b++){
    const B=bosses[b];
    const ex = B.x - px, ey = B.y - py;
    const t = Math.max(0, Math.min(maxLen, ex*vx + ey*vy));
    const hx = px + vx*t, hy = py + vy*t;
    const dx = B.x - hx, dy = B.y - hy;
    if(dx*dx + dy*dy <= (B.size+8)*(B.size+8)){
      if(!P.laserPierce){
        if(t < firstT){ firstT = t; firstIdx = {boss:b}; firstPoint = {x:hx,y:hy}; }
      }else{
        const now = tNow;
        if(L._cool['b'+b] && now < L._cool['b'+b]) continue;
        L._cool['b'+b] = now + 0.05;
        B.hp -= P.laserDamage*0.6;
        hitFlash(B.x,B.y,B.size*0.6);
        if(B.hp<=0){
        spawnExplosion(B.x,B.y,B.size*1.2,true);
        addScore(200);
        addShake(2.5);
        sfxExpl('big');
        bosses.splice(b,1);
  ...
  if(!P.laserPierce && firstT !== Infinity){
    // Apply single-target damage at firstPoint
    if(firstIdx && typeof firstIdx==='object'){
      const b = firstIdx.boss;
      if(bosses[b]){
        bosses[b].hp -= P.laserDamage*0.6;
        hitFlash(firstPoint.x, firstPoint.y, bosses[b].size*0.6);
        if(bosses[b].hp<=0){
          spawnExplosion(bosses[b].x,bosses[b].y,bosses[b].size*1.2,true);
          addScore(200);
          bosses.splice(b,1);
          addShake(2.5);
          sfxExpl('big');
    ...
    }else if(firstIdx>=0 && enemies[firstIdx]){
      const E=enemies[firstIdx];
      E.hp -= P.laserDamage;
      hitFlash(firstPoint.x, firstPoint.y, E.size*0.7);
      if(E.hp<=0){
        spawnExplosion(E.x,E.y,E.size);
        addScore(E.value||10);
        enemies.splice(firstIdx,1);
      }
    }
    // render length cutoff at impact
    L.renderLen = firstT;
  }else{
    L.renderLen = Math.max( (farthestT||0), maxLen*0.8 );
  }
}
// enemies update + collisions
  for(let i=enemies.length;i--;){
    ...
    if(gameState==='playing'){
      for(let j=bullets.length;j--;){
        ...
        if(d2b < rr*rr){
          // tính damage theo người bắn
          const ownerId = (b.owner==='p1')?1:0;
          const P = players[ownerId]||players[0];
          e.hp -= (P.bulletDmg||8);
          puff(b.x,b.y,`hsla(${e.hue},90%,60%,1)`,8,2.2,0.45,0.9);
          bullets.splice(j,1); addShake(0.4);
          if(e.hp<=0){
            puff(e.x,e.y,`hsla(${e.hue},95%,60%,1)`,22,3.0,0.9,1.2);
      ...
      for(const p of players){
        ...
        if(d2p < rr*rr){
          p.hp -= 12;
          puff((e.x+p.x)/2,(e.y+p.y)/2,'rgba(255,120,120,0.95)',18,3.5,0.8,1.0);
    ...
    for(let i=bullets.length;i--;){
      ...
      for(const p of players){
        ...
        if(d2p < rr*rr){
          ...
          puff(p.x,p.y,'rgba(255,200,120,0.95)',12,3.2,0.6,0.9);
    ...
    if(gameState==='playing'){
      for(let j=bullets.length;j--;){
        ...
        if(d2b<rr*rr){
          ...
          puff(b.x,b.y,`hsla(${B.hue},90%,60%,1)`,8,3.2,0.45,0.9);
          bullets.splice(j,1); addShake(1.0);
          if(B.hp<=0){
            puff(B.x,B.y,`hsla(${B.hue},90%,60%,1)`,72,7.0,1.6,1.6);
  ...
  // particles
  for(let i=particles.length;i--;) if((particles[i].life-=0)<=0) particles.splice(i,1); else particles[i].update(dt);
  ...
  // deaths / gameover
  for(const p of players){ if(p.alive && p.hp<=0){ p.alive=false; p.vx=0; p.vy=0; } }
  if(gameState==='playing'){
    const anyAlive = players.some(p=>p.active && p.alive);
    if(!anyAlive){
      gameState='gameover';
      // thưởng điểm vào kho (50% tổng score)
      const gain = Math.round(totalScore()*0.5);
      save.totalPoints = (save.totalPoints||0) + gain;
      saveSave(save); updateUI();
      toMenuBGM();
...
function drawShip(p){
  ...
  ctx.fillStyle=g; ctx.beginPath(); ctx.moveTo(-p.size, -p.size*0.6);
  ctx.lineTo(p.size*1.1, 0); ctx.lineTo(-p.size, p.size*0.6); ctx.closePath(); ctx.fill();
...
function render(){
  ...
  for(const L of lasers){
    const a=Math.atan2(L.ty-L.y,L.tx-L.x);
    const lx=Math.cos(a), ly=Math.sin(a);
    const len=(L.renderLen!==undefined?L.renderLen:1600);
  ...
  // players
  for(const p of players){ if(p.active){ if(p.alive) drawShip(p); else { ctx.save(); ctx.globalAlpha=0.5; ctx.fillStyle='rgba(10,0,16,0.5)'; ctx.beginPath(); ctx.arc(p.x,p.y,70,0,Math.PI*2); ctx.fill(); ctx.restore(); } } }
  // bosses
...
function resetPlayersPositions(){
  ...
  applyBuffs(); // reapply upgrades to fresh players
}
...
// ---------------------- SHOP (Upgrades) ----------------------
// Chỉ lưu "save.totalPoints" + "save.upgrades" vào localStorage.
// Nút shop sinh trong #shop. Mua xong auto save + cập nhật Points hiển thị.
function buildShopUI(){
  ...
  const addBtn=(label,cost,fn,disabled=false)=>{
    ...
    b.addEventListener('click', ()=>{
      ...
      fn&&fn();
      applyBuffs(); 
      saveSave(save); // lưu upgrades sau mỗi lần mua
      updateUI();
      buildShopUI();
    });
    el.appendChild(b);
...
function applyBuffs(){
  ...
  // Bullet
  // fireRate càng thấp càng nhanh -> 0.12 base, mỗi cấp giảm ~4% (giới hạn)
  players.forEach(p=>{
...
(function(){
  ...
  if(!window.returnToMenu){
    window.returnToMenu = function(){
      ...
      try{ gameState='menu'; toMenuBGM(); updateUI(); }catch{}
    };
  }
})();
