...
let singleMode = true; // default, overwritten by menu radio
...
class Enemy {
  ...
  update(dt){
    ...
    if(this.shootTimer <= 0){
      ...
      const aliveTargets = players.filter(pp=>pp.enabled!==false && pp.alive);
  ...
  update(dt){
    ...
    if(this.shootTimer <= 0){
      if(this.phase === 1){
        ...
        for(let h=0; h<4; h++){
          const aliveTargets3 = players.filter(pp=>pp.enabled!==false && pp.alive);
          if(aliveTargets3.length===0) continue;
          const target = aliveTargets3.reduce((a,b)=> (a && dist(a.x,a.y,this.x,this.y) < dist(b.x,b.y,this.x,this.y)) ? a : b );
...
function update(){
  ...
  if(players[1].enabled!==false && players[1].alive && (adx!==0 || ady!==0)){ const m = Math.hypot(adx,ady)||1; players[1].aimDir.x = adx/m; players[1].aimDir.y = ady/m; }

  // damping & movement
  for(const p of players){
    if(!(p.enabled!==false && p.alive)) { p.vx=0; p.vy=0; continue; }
    p.vx *= 0.86; p.vy *= 0.86;
  ...
  // Player firing
  players[0].reload -= dt;
  if(!(players[0].enabled!==false && players[0].alive)) {} else {
  const aimDx = mouse.x - players[0].x, aimDy = mouse.y - players[0].y;
  const aimAng = Math.atan2(aimDy, aimDx);
  if((mouse.down || keys[' ']) && players[0].reload <= 0 && (players[0].enabled!==false && players[0].alive)){
  ...
  // player2 fire with 'm'
  players[1].reload -= dt;
  if(!(players[1].enabled!==false && players[1].alive)) { /* skip */ } else {
  ...
  for(let i=bullets.length-1;i>=0;i--){
    ...
    if(b.homing){
      // find nearest player
      const aliveTargetsH = players.filter(pp=>pp.enabled!==false && pp.alive);
      const target = aliveTargetsH.length? aliveTargetsH.reduce((a,bp)=> (a && dist(a.x,a.y,b.x,b.y) < dist(bp.x,bp.y,b.x,b.y)) ? a : bp ) : null;
      if(target){
  ...
  // spawn waves
  waveTime += dt;
  if(gameState==='playing' && waveTime > spawnInterval){ spawnWave(); difficulty += 0.02; }

  // sometimes spawn boss when difficulty enough or score milestone
  if(gameState==='playing' && Math.random() < 0.0009 * difficulty && bosses.length === 0 && (players[0].score + players[1].score) > 120 + difficulty*50){
    bosses.push(new Boss(W/2, -120)); addShake(2); spawnParticles(W/2, 60, 'rgba(255,120,200,0.9)', 60, Math.PI*2, 8, 1.4);
  }

  // player death
  for(const p of players){ if(p.hp <= 0 && p.alive){ p.alive = false;
    p.vx=0; p.vy=0; p.vx=0; p.vy=0; addShake(8); } }
  const enabledCount = players.filter(pp=>pp.enabled!==false).length;
  const aliveEnabled = players.filter(pp=>pp.enabled!==false && pp.alive).length;
  if(gameState==='playing' && enabledCount>0 && aliveEnabled===0){ gameState='gameover'; }
...
function render(){
  ...
  if(players[0].enabled!==false && players[0].alive){
  ctx.save();
  ...
  ctx.save();
  if(players[1].enabled!==false && players[1].alive){
  ...
  // game over big
  const aliveEnabledNow = players.filter(pp=>pp.enabled!==false && pp.alive).length;
  const anyEnabled = players.some(pp=>pp.enabled!==false);
  if(anyEnabled && aliveEnabledNow===0){
    ctx.save();
...
document.getElementById('startBtn').addEventListener('click', ()=> { 
  ...
  singleMode = !modeEl || modeEl.value==='single';
  players[0].enabled = true;
  players[1].enabled = !singleMode;
  if(singleMode){ players[1].alive=false; players[1].vx=0; players[1].vy=0; }
  menu.style.display = 'none';
  restart();
});
document.getElementById('restartBtn').addEventListener('click', ()=> { 
  if(singleMode){ players[1].enabled=false; }
  restart();
});
