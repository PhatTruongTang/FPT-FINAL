// ====== Neon Shooter - main_upgraded.js (fixed) ======
// Prevent auto-run behind menu + proper Single/Co-op handling + target alive/active players only
...
if (!window.__GalaxyShooterInitialized) {
  ...
  (function () {
    ...
    // ---- Save stub (kept from previous version) ----
    ...
    // --- Game state controls ---
    let playing = false;      // <- NEW: no updates spawn until startGame()
    let gameMode = 'coop';    // 'single' | 'coop'
    let keys = {};
    ...
    function spawnParticles(x, y, color, count = 12, spread = Math.PI, speed = 0.8, size = 3, life = 0.8) {
      for (let i = 0; i < count; i++) {
        const a = rnd(-spread / 2, spread / 2) + rnd(0, Math.PI * 2);
        const s = rnd(speed * 0.2, speed);
        particles.push(new Particle(x, y, Math.cos(a) * s, Math.sin(a) * s, rnd(size * 0.4, size), life, color));
      }
    }
    ...
    const players = [
      { id: 0, x: W * 0.5, y: H * 0.7, vx: 0, vy: 0, speed: 420, size: 18, hp: 120, maxHp: 120, reload: 0, fireRate: 0.09, score: 0, alive: true, active: true, skin: 0, colorHue: 190 },
      { id: 1, x: W * 0.4, y: H * 0.8, vx: 0, vy: 0, speed: 380, size: 18, hp: 120, maxHp: 120, reload: 0, fireRate: 0.18, score: 0, alive: true, active: true, skin: 1, colorHue: 320, aimDir: { x: 1, y: 0 } },
    ];
    ...
    // Utility to get alive+active players
    function getActivePlayers() {
      return players.filter((p) => p.active && p.alive);
    }
    function getTargetFor(x, y) {
      const avail = players.filter((p) => p.active && p.alive);
      if (avail.length === 0) return null;
      let best = avail[0];
      let bd = dist(x, y, best.x, best.y);
      for (let i = 1; i < avail.length; i++) {
        const d = dist(x, y, avail[i].x, avail[i].y);
        if (d < bd) {
          best = avail[i];
          bd = d;
        }
    ...
    class Enemy {
      constructor(x, y, type = 0) {
        ...
        this.y = y;
        this.vx = 0;
        this.vy = 0.02 + Math.random() * 0.1;
        this.size = 14 + Math.random() * 12;
        this.hp = Math.round(8 + this.size);
        this.type = type;
        this.angle = 0;
        this.shootTimer = rnd(1.6, 3.2);
        this.speedMultiplier = 1 + Math.random() * 0.6;
        this.hue = maps[selectedMap].bgHue + 80 - Math.random() * 160;
      }
      update(dt) {
        this.angle += dt * 0.2;
        ...
        if (this.shootTimer <= 0) {
          ...
          const target = getTargetFor(this.x, this.y);
          if (target) {
    ...
    class Boss {
      constructor(x, y) {
        ...
        this.y = y;
        this.size = 64;
        this.maxHp = 220 + Math.round(difficulty * 80);
        this.hp = this.maxHp;
        this.phase = 1;
        this.timer = 0;
      ...
      update(dt) {
        ...
        if (this.hp < this.maxHp * 0.65 && this.phase === 1) {
          ...
          this.shootTimer = 0;
          addShake(1.2);
          spawnParticles(this.x, this.y, 'rgba(255,200,120,0.9)', 30, Math.PI * 2, 6, 1.2);
        }
        if (this.hp < this.maxHp * 0.35 && this.phase === 2) {
          this.phase = 3;
          this.shootTimer = 0;
          addShake(1.8);
          spawnParticles(this.x, this.y, 'rgba(255,80,120,0.95)', 48, Math.PI * 2, 8, 1.6);
        }
        // shooting logic by phase
        this.shootTimer -= dt;
        const targetExists = getActivePlayers().length > 0;
        if (this.shootTimer <= 0 && targetExists) {
          if (this.phase === 1) {
            ...
            // homing bullets (towards nearest alive+active player)
            for (let h = 0; h < 4; h++) {
              const tgt = getTargetFor(this.x, this.y);
              if (!tgt) break;
      ...
      draw(ctx) {
        ...
        ctx.fill();
        // turret
        ctx.fillStyle = `hsla(${this.hue + 30},90%,55%,0.95)`;
        ctx.beginPath();
        ctx.rect(s * 0.6, -s * 0.18, s * 0.9, s * 0.36);
        ctx.fill();
        ctx.restore();
        // HP bar
        const hbX = W / 2 - 240,
        ...
        grd.addColorStop(0.6, '#ffdd66');
        grd.addColorStop(1, '#6bffea');
        ctx.fillStyle = grd;
    ...
    function update() {
      ...
      // If not playing, only do super-light UI/particles decay and skip gameplay
      if (!playing) {
        // light particle decay for menu fx
      ...
      if (players[0].active && players[0].alive) {
        ...
        if (mag1 > 0) {
          mx1 /= mag1;
          my1 /= mag1;
          players[0].vx += mx1 * players[0].speed * dt * 3;
          players[0].vy += my1 * players[0].speed * dt * 3;
        }
      }
      ...
      if (players[1].active && players[1].alive) {
        const left2 = keys['arrowleft'],
          right2 = keys['arrowright'],
          up2 = keys['arrowup'],
          down2 = keys['arrowdown'];
        let mx2 = 0,
          my2 = 0;
        if (left2) mx2 -= 1;
        if (right2) mx2 += 1;
        ...
        if (mag2 > 0) {
          mx2 /= mag2;
          my2 /= mag2;
          players[1].vx += mx2 * players[1].speed * dt * 3;
          players[1].vy += my2 * players[1].speed * dt * 3;
        }
        ...
        if (aimDown) ady += 1;
        if (adx !== 0 || ady !== 0) {
          const m = Math.hypot(adx, ady) || 1;
          players[1].aimDir.x = adx / m;
          players[1].aimDir.y = ady / m;
        }
      }
      ...
      if (players[0].active && players[0].alive) {
        players[0].reload -= dt;
        const aimDx = mouse.x - players[0].x,
          aimDy = mouse.y - players[0].y;
        const aimAng = Math.atan2(aimDy, aimDx);
        if ((mouse.down || keys[' ']) && players[0].reload <= 0) {
      ...
      for (let i = bullets.length - 1; i >= 0; i--) {
        const b = bullets[i];
        if (b.homing) {
          const target = getTargetFor(b.x, b.y);
          if (target) {
      ...
      for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        if (e.y > H + 120) {
          enemies.splice(i, 1);
          for (const p of players) if (p.active) p.score = Math.max(0, p.score - 6);
          continue;
        }
        for (let j = bullets.length - 1; j >= 0; j--) {
          ...
          if (dist(b.x, b.y, e.x, e.y) < e.size + b.size) {
            ...
            addShake(0.6);
            if (e.hp <= 0) {
              spawnParticles(e.x, e.y, `hsla(${e.hue},90%,60%,1)`, 24, Math.PI * 2, 3.2, 0.9);
              for (const p of players) if (p.active) p.score += Math.round(4 + e.size * (p.id === 0 ? 1.0 : 0.5));
              enemies.splice(i, 1);
              break;
            }
          }
        ...
        for (let j = bullets.length - 1; j >= 0; j--) {
          ...
          if (dist(b.x, b.y, B.x, B.y) < B.size + b.size) {
            ...
            if (B.hp <= 0) {
              ...
              for (const p of players) if (p.active) p.score += p.id === 0 ? 200 : 160;
              bosses.splice(i, 1);
      ...
      // spawn waves
      waveTime += dt;
      if (waveTime > spawnInterval) {
        spawnWave();
        difficulty += 0.02;
      }

      // maybe spawn boss
      const totalScore = players.reduce((a, p) => a + (p.active ? p.score : 0), 0);
      if (Math.random() < 0.0009 * difficulty && bosses.length === 0 && totalScore > 120 + difficulty * 50) {
        bosses.push(new Boss(W / 2, -120));
        addShake(2);
        spawnParticles(W / 2, 60, 'rgba(255,120,200,0.9)', 60, Math.PI * 2, 8, 1.4);
      }
      ...
      // game over when all ACTIVE players are down
      const active = players.filter((p) => p.active);
      const allDown = active.length > 0 && active.every((p) => !p.alive);
      if (allDown) {
        // freeze spawns but keep render
        paused = true;
      }
    ...
    function drawShip(ctx, p) {
      ...
      const ang = p.id === 0 ? Math.atan2(mouse.y - py, mouse.x - px) : Math.atan2(p.aimDir.y, p.aimDir.x);
      ...
      ctx.beginPath();
      ctx.ellipse(-p.size * 0.35, 0, p.size * 0.45, p.size * 0.35, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.globalAlpha = 0.9;
      ctx.fillStyle = '#7af';
      ctx.beginPath();
      ctx.ellipse(-p.size * 0.9, 0, p.size * 0.4, p.size * 0.25, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
      ctx.globalAlpha = 1;
    }
    ...
    function render() {
      ...
      if (players[0].active) {
        ...
        ctx.fillText(Math.round(players[0].hp) + ' / ' + players[0].maxHp, hbX + hbW + 8, hbY + hbH - 1);
      }
      // P2
      if (players[1].active) {
        const hbW = 220,
          ...
          hbY2 = 60;
        ctx.fillStyle = '#222';
        ctx.fillRect(hbX2, hbY2, hbW, hbH);
        const pct2 = clamp(players[1].hp / players[1].maxHp, 0, 1);
        const grd2 = ctx.createLinearGradient(hbX2, 0, hbX2 + hbW, 0);
        grd2.addColorStop(0, '#ff6b6b');
        grd2.addColorStop(0.6, '#ffdd66');
        grd2.addColorStop(1, '#6bffea');
        ctx.fillStyle = grd2;
        ctx.fillRect(hbX2, hbY2, hbW * pct2, hbH);
        ctx.strokeStyle = 'rgba(255,255,255,0.08)';
        ctx.strokeRect(hbX2, hbY2, hbW, hbH);
        ctx.fillStyle = 'rgba(255,255,255,0.7)';
        ctx.font = '12px Inter, Arial';
        ctx.fillText(Math.round(players[1].hp) + ' / ' + players[1].maxHp, hbX2 - 90, hbY2 + hbH - 1);
      }
      ctx.restore();

      // crosshair for P1 if active
      if (players[0].active) {
        ctx.save();
        ctx.globalAlpha = 0.8;
        ctx.fillStyle = '#9ff';
      ...
      // small P2 aim indicator if active
      if (players[1].active) {
        ctx.save();
        const p2aimX = players[1].x + players[1].aimDir.x * 28;
        const p2aimY = players[1].y + players[1].aimDir.y * 28;
        ctx.globalAlpha = 0.85;
        ctx.fillStyle = '#ffd';
        ctx.beginPath();
        ctx.arc(p2aimX, p2aimY, 6, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
      }

      // Game over banner only if playing and all active down
      if (playing) {
        const active = players.filter((p) => p.active);
        const allDown = active.length > 0 && active.every((p) => !p.alive);
        if (allDown) {
          const p1s = players[0].active ? players[0].score : 0;
          const p2s = players[1].active ? players[1].score : 0;
          ctx.save();
          ctx.fillStyle = 'rgba(4,0,6,0.6)';
          ctx.fillRect(0, 0, W, H);
          ...
          ctx.shadowColor = '#ff6b9b';
          ctx.shadowBlur = 24;
          ctx.fillText('ALL PLAYERS DOWN', W / 2, H / 2 - 24);
          ctx.shadowBlur = 0;
          ctx.font = '600 18px Inter, Arial';
          ctx.fillStyle = '#bfe';
          ctx.fillText(`Final Scores — P1: ${p1s}` + (players[1].active ? ` • P2: ${p2s}` : '') + ' • Press R to restart', W / 2, H / 2 + 18);
          ctx.restore();
        }
      }
    }
    ...
    function updateUI() {
      ...
      if (hudPlayers)
        hudPlayers.innerHTML = `<div style="color:#bff">Mode: ${gameMode.toUpperCase()} • P1 skin: ${players[0].skin}` + (players[1].active ? ` • P2 skin: ${players[1].skin}` : '') + ` • Map: ${maps[selectedMap].name}</div>`;
    ...
    let lastInit = performance.now() / 1000;
    lastInit = performance.now() / 1000;
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (k === 'p' && playing) {
        // only pause while playing
        e.preventDefault();
        const was = paused;
        paused = !paused;
        if (paused && UI) UI.innerHTML += '<div style="color:#fdd"> • PAUSED</div>';
    ...
    function restart() {
      ...
      particles = [];
      bosses = [];
      // reset active players according to gameMode
      players[0].active = true;
      players[1].active = gameMode === 'coop';
      // reset state per player
      players.forEach((p, i) => {
        p.x = W * (0.5 - (p.active ? i * 0.1 : 0));
        p.y = H * (0.7 + (p.active ? i * 0.05 : 0));
        p.vx = 0;
        p.vy = 0;
        p.hp = p.maxHp;
        p.alive = p.active; // inactive -> considered down/hidden
        p.reload = 0;
        p.score = 0;
      });
      difficulty = 1;
    ...
    /* Start helpers */
    function startGame(mode = 'coop') {
      gameMode = mode;
      // hide menu if exists
      if (menu) menu.style.display = 'none';
      // set flags before restart, then restart & play
      players[0].active = true;
      players[1].active = mode === 'coop';
      restart();
      playing = true;
    }
    ...
    const m0 = document.querySelector('[data-map="0"]');
    const m1 = document.querySelector('[data-map="1"]');
    const m2 = document.querySelector('[data-map="2"]');
    if (m0) m0.style.background = 'linear-gradient(180deg,#05010a,#090010)';
    if (m1) m1.style.background = 'linear-gradient(180deg,#20000a,#4a0018)';
    if (m2) m2.style.background = 'linear-gradient(180deg,#001a2e,#002b46)';

    // --- Start buttons (compatible with older HTML) ---
    const startBtn = document.getElementById('startBtn');
    const startSingleBtn = document.getElementById('startSingleBtn') || document.getElementById('singleBtn');
    const startCoopBtn = document.getElementById('startCoopBtn') || document.getElementById('coopBtn');

    if (startSingleBtn) startSingleBtn.addEventListener('click', () => startGame('single'));
    if (startCoopBtn) startCoopBtn.addEventListener('click', () => startGame('coop'));
    if (startBtn) startBtn.addEventListener('click', () => startGame('coop')); // default behavior
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.hidden && playing) paused = true;
    });
